...
# Import Libraries due to DataConversionWarning
# (source: https://stackoverflow.com/questions/29086398/sklearn-turning-off-warnings)
import warnings
...
if __name__ == '__main__':
    ...
    train_csv = dataset[0:math.ceil(0.85 * len(dataset))]
    test_csv = dataset[math.ceil(0.85 * len(dataset)):]

    # Load train examples, X using list and map, accordingly -> use [list(map(float, .. ))] to modify to float data
    X = [list(map(float, train_csv[j][:-1])) for j in range(0, len(train_csv))]

    # Load train examples, y using list and map, accordingly -> use [list(map(float, .. ))] to modify to float data
    y = [list(map(float, train_csv[j][-1])) for j in range(0, len(train_csv))]
    ...
    # Transform the test_csv in order to evaluate the accuracy -> use [list(map(float, .. ))] to modify to float data
    test_x = [list(map(float, test_csv[j][:-1])) for j in range(0, len(test_csv))]

    # Add up accuracy using predict function
    for j in range(0, len(test_csv)):
        predict = clf.predict([test_x[j]])
        if predict[0] == float(test_csv[j][-1]):
            acc += 1

    # Print accuracy score
    print(acc / len(test_csv))

    # Split input entry with list comprehension -> use float() cast to modify data to float values
    input_entry = [[float(elem) for elem in input().split(' ')]]

    # Print results
    print(int(clf.predict(input_entry)[0]))
