...
def d2(ind, state):
    temp = list(state)[:]
    temp[ind + 2] = state[ind]
    temp[ind] = 0
    return temp


class Disks(Problem):
    def __init__(self, N, L, init, goal=None):
    ...
    def successor(self, state):
        ...
        for i in range(self.L):
            if state[i] != 0:
                # L1
                if self.l1_valid(i, state):
                    successors["L1: Disk %s" % (state[i])] = tuple(l1(i, state))

                # D1
                if self.d1_valid(i, state):
                    successors[f"D1: Disk %s" % (state[i])] = tuple(d1(i, state))

                # L2
                if self.l2_valid(i, state):
                    successors[f"L2: Disk %s" % (state[i])] = tuple(l2(i, state))

                # D2
                if self.d2_valid(i, state):
                    successors[f"D2: Disk %s" % (state[i])] = tuple(d2(i, state))
    ...
    def isvalid(self, index):
        if 0 <= index and index < self.L:
            return True
        else:
            return False

    def l1_valid(self, index, state):
        if self.isvalid(index - 1):
            if state[index - 1] == 0:
                return True
        return False

    def d1_valid(self, index, state):
        if self.isvalid(index + 1):
            if state[index + 1] == 0:
                return True
        return False

    def l2_valid(self, index, state):
        if self.isvalid(index - 2):
            if state[index - 2] == 0 and state[index - 1] != 0:
                return True
        return False

    def d2_valid(self, index, state):
        if self.isvalid(index + 2):
            if state[index + 2] == 0 and state[index + 1] != 0:
                return True
        return False
...
if __name__ == '__main__':
    ...
    for index in range(1, N + 1):
        initial[index-1] = index
    goal = tuple(initial[::-1])
    obj = Disks(N, L, tuple(initial), goal)
    result = breadth_first_graph_search(obj)
