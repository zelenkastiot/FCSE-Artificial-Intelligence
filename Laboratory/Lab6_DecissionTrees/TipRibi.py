...
# Get column to be excluded from user
col_index = int(input())

# get number of estimators for Random Forest from user
est = int(input())

# Get criterion from user
criterion = input()

# Create empty list to store user sample
user_samp = []

# Split input sample on empty spaces
input_string = input().split(' ')

# For each element slitted on ' ' use float() to cast its value and store in in user_samp
for i in range(len(input_string)):
    user_samp.append(float(input_string[i]))

# Use function pop to delete column in our sample specified by user
user_samp.pop(col_index)

# Loop dataset and use function pop to delete column from every sample in dataset
for i in dataset:
    i.pop(col_index)

# Split dataset (85% train, 15% test)
train_csv = dataset[:int(0.85 * len(dataset))]
test_csv = dataset[int(0.85 * len(dataset)):]
...
classifier = RandomForestClassifier(n_estimators=est, criterion=criterion, random_state=0)
...
# Calculate probability (prob) and prediction (pred)
prob = classifier.predict_proba([user_samp])[0]
pred = classifier.predict([user_samp])[0]
